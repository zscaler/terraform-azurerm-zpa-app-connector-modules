...
#####################################################################################################################
## By default this module onboards App Connectors using OAuth2 user codes (recommended). The user code is read
## from /etc/issue at boot, published to an Azure Key Vault via the VM's Managed Identity, and read back by
## Terraform to create the App Connector Group. Set onboarding_method to "provisioning_key" to use the legacy
...
## How long (seconds) to wait for connectors to publish their OAuth2 user codes to Key Vault before reading back.
